...
function CalendarDropdown({ value, onChange, options = [], 'aria-label': ariaLabel }: DropdownProps): React.JSX.Element {
  const handleValueChange = React.useCallback(
    (val: string) => {
      if (onChange == null) {
        return
      }
      // react-day-picker espera un ChangeEvent<HTMLSelectElement>
  ...
  return (
    <Select value={String(value ?? '')} onValueChange={handleValueChange}>
      <SelectTrigger className="h-8 w-auto gap-1 border-0 bg-transparent px-2 py-1 text-sm font-medium shadow-none focus:ring-0" aria-label={ariaLabel}>
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        {options.map((opt) => (
  ...
  const mergedComponents = React.useMemo(
    () => ({
      Root: ({
        ...
        rootRef,
        ...rootProps
      }: { className?: string; rootRef?: React.Ref<HTMLDivElement> } & React.HTMLAttributes<HTMLDivElement>) => {
      ...
      WeekNumber: ({ children: weekChildren, ...weekProps }: { children?: React.ReactNode } & React.TdHTMLAttributes<HTMLTableCellElement>) => {
        return (
          <td {...weekProps}>
            <div className="flex size-(--cell-size) items-center justify-center text-center">{weekChildren}</div>
          </td>
        )
      },
      ...components
...
function CalendarDayButton({ className, day, modifiers, ...props }: React.ComponentProps<typeof DayButton>): React.JSX.Element {
  const defaultClassNames = getDefaultClassNames()
